...
public class LevelOne extends LevelParent {
			...
			public void mouseClicked(java.awt.event.MouseEvent e) {
				...
				try {
					...
					AudioInputStream audioInputStream = AudioSystem
							.getAudioInputStream(new File("resources/sound/mouseClick.wav").getAbsoluteFile());
					// Get audio clip
				...
				if (gameRunning)
					game.addClick(false); // Add a false click
	...
	public void finishGame(boolean win) {
		...
		if (win)
			buttonCont.add(continueBtn); // Allow them to continue to the next level
		else { // User loses
			BorderLayout layout = (BorderLayout) firstLevel.getLayout(); // Remove bottom component
			firstLevel.remove(layout.getLayoutComponent(BorderLayout.SOUTH));
			buttonCont.add(retryBtn); // Add retry button
		}
	...
	public void buttonClicked(ActionEvent e) {
		...
		if (compare == retryBtn) {
			...
			startGame(); // Start game (timer and clicking)
		}
