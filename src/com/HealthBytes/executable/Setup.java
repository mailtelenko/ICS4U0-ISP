...
/**
 * The Setup class prompts the user to enter a name input and verifies the
 * highscore.txt file is accessible.
 * 
...
public class Setup extends MenuParent {
	...
	public Setup(Game gme) {
		...
		game.resetClicks(); // Set clicks to 0 (new game)
	...
	public void buttonClicked(ActionEvent e) {
		...
		if (compare == cont) { // Check if the button is the same as continue
			if ((name = textField.getText()).length() < 1 || textField.getText().equals(" ")) { // Check if name is
				...
				game.setName(name); // Set name
				game.resetClicks(); // Reset clicks (0)
				game.resetTime(); // Reset time (0)
				game.resetAnswers(); // Reset correct/incorrect answers
