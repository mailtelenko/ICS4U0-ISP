...
public class LevelThree extends LevelParent {
	...
	/**
	 * LevelThree class constructor sets the game reference to the game object and
	 * creates the panel for use.
	 * 
	 * @param gme
	...
	public void updateButtons() {
		...
		JButton exitButton = new JButton("Block and report User " + currentUser);
	...
	public void checkButtons(String clickedString) {
		...
		if (clickedString.equals("Block and report User " + correctUser)) {
			finishGame(true);
			showInfoPane(
					new String[] { "Correct User", "You managed to identify the suspicious individual. Good job!" });
		} else if (clickedString.equals("Block and report User 1") || clickedString.equals("Block and report User 2")
				|| clickedString.equals("Block and report User 3")) {
			showInfoPane(new String[] { "Incorrect User",
					"The selected user was not the suspicious individual and you let the culprit get away. Please retry the level." });
			finishGame(false);
			fillPrompts();
		}
	...
	public void buttonClicked(ActionEvent e) {
		...
		if (compare == retryBtn) { // If retry is clicked
			...
			game.window.getContentPane().add(new LevelThree(game)); // Add levelThree to panels
		} else if (compare == continueBtn) { // If continue is clicked
			game.window.getContentPane().removeAll(); // Remove all panels from JFrame
			game.window.getContentPane().add(new Quiz(game, 3)); // Add new Quiz (level three) to panels
