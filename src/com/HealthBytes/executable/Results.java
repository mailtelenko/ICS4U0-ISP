...
/**
 * The Results class displays the score of the user's playthrough based on a
 * number of factors collected over the levels. It also writes it to the
 * highscores.txt file.
 * 
...
public class Results extends MenuParent {
	...
	public Results(Game gme) {
		...
		// Add data to panel
		resultsPanel.add(resultsLabel = new JLabel(
				"<html>" + "<center>Final Score: " + score + "<br/><br/><br/><br/>Total Time: " + game.getTime()
						+ " seconds<br/><br/>Incorrect Answers: " + game.getIncorrectAnswers()
						+ "<br/><br/>Incorrect Clicks: " + game.getIncorrectClicks() + "</center>" + "</html>"));
	...
	/**
	 * writeToHighScores uses insertion sort to find the top 10 scores and writes
	 * them to highScores.txt file
	 */
	private void writeToHighScores() {
		ArrayList<Integer> scoresNum = new ArrayList<Integer>(); // Create new ArrayList to store previous scores that
																	// exclude names
		...
		try {
			...
			PrintWriter dataOut = new PrintWriter(
					new FileWriter(game.getHighScoresPath() + "\\CyberCase\\highscores.txt"));
	...
	/**
	 * createButtons method creates a mainMenu button to return to mainMenu, and a
	 * highScores button to view high scores.
	 * 
	 * @return JPanel with buttons wrapped in the center.
	 */
