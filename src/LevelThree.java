...
public class LevelThree extends LevelParent {
	 ...
	 */
	public LevelThree(Game gme, int t, int ia, int ic) {
		super(gme, "levelThree"); // Call to super with level
		game = gme; // Set reference to game object

		// Set instance variables
		game = gme; // Set reference to game object
		totalTime = t; //Set total time (from previous level)
		incorrectAnswers = ia; //Set incorrect answers (from previous level)
		incorrectClicks = ic; // Set incorrectClicks (from previous level)
	...
	public void buttonClicked(ActionEvent e) {
		...
		if (compare == menu) { // Check if the clicked button is the same object as menu button
			...
			game.window.getContentPane().removeAll(); // Remove all panels from JFrame
			game.window.getContentPane().add(new LevelThree(game, totalTime, incorrectAnswers, incorrectClicks)); // Add mainMenu to panels
		} else if (compare == continueBtn) {
			game.window.getContentPane().removeAll(); // Remove all panels from JFrame
			game.window.getContentPane().add(new Quiz(game, 3, totalTime, incorrectAnswers, incorrectClicks)); // Add mainMenu to panels
