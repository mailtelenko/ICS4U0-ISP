...
public class Setup extends MenuParent {
  ...
  public void buttonClicked(ActionEvent e) {
    ...
    if (compare == cont) {
    	if ((name = textField.getText()).length() < 1) { //Confirm erase with user
    		JOptionPane.showMessageDialog(game.window, "Please enter a name before continuing.",
    				"Enter name", JOptionPane.ERROR_MESSAGE);
    	}else{
	      System.out.println("Continue" + name);
	      game.window.getContentPane().removeAll();
	      game.window.getContentPane().add(new LevelOne(game));
    	}
    }
